import { Gauge, Plus, Timer, Trash2 } from "lucide-react";
import type { MachinePoint } from "../types/machine";

type PointsTableProps = {
  points: MachinePoint[];
  onChange: (points: MachinePoint[]) => void;
};

const numberOrZero = (value: string) => {
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : 0;
};

const defaultPointSettings = {
  downSpeedMmPerSecond: 5,
  holdDownSeconds: 10,
  upSpeedMmPerSecond: 5,
  holdUpSeconds: 5
};

export function PointsTable({ points, onChange }: PointsTableProps) {
  const updatePoint = (id: string, patch: Partial<MachinePoint>) => {
    onChange(points.map((point) => (point.id === id ? { ...point, ...patch } : point)));
  };

  const addPoint = () => {
    const nextNumber = points.length + 1;
    onChange([
      ...points,
      {
        id: crypto.randomUUID(),
        name: `Ponto ${nextNumber}`,
        x: 0,
        y: 0,
        repetitions: 1,
        ...defaultPointSettings
      }
    ]);
  };

  const removePoint = (id: string) => {
    onChange(points.filter((point) => point.id !== id));
  };

  return (
    <section className="space-y-4">
      <div className="section-titlebar">
        <div>
          <span className="section-kicker">Mapa da rotina</span>
          <h2 className="section-heading">Pontos, velocidades e tempos</h2>
        </div>
        <button className="btn-secondary" type="button" onClick={addPoint}>
          <Plus size={17} />
          Adicionar ponto
        </button>
      </div>

      <div className="flex flex-wrap gap-2">
        <span className="unit-pill">
          <Gauge size={14} />
          mm/s
        </span>
        <span className="unit-pill">
          <Timer size={14} />
          segundos
        </span>
      </div>

      <div className="table-shell">
        <table className="machine-table">
          <thead>
            <tr>
              <th className="w-12">#</th>
              <th className="px-3 py-3 font-semibold">Nome</th>
              <th className="px-3 py-3 font-semibold">X</th>
              <th className="px-3 py-3 font-semibold">Y</th>
              <th className="px-3 py-3 font-semibold">Repeticoes</th>
              <th className="px-3 py-3 font-semibold">Descida (mm/s)</th>
              <th className="px-3 py-3 font-semibold">Embaixo (s)</th>
              <th className="px-3 py-3 font-semibold">Subida (mm/s)</th>
              <th className="px-3 py-3 font-semibold">Em cima (s)</th>
              <th className="w-14 px-3 py-3 font-semibold"></th>
            </tr>
          </thead>
          <tbody>
            {points.map((point, index) => (
              <tr key={point.id} className="bg-panel">
                <td>
                  <span className="row-index">{index + 1}</span>
                </td>
                <td className="px-3 py-2">
                  <input className="table-input" value={point.name} onChange={(event) => updatePoint(point.id, { name: event.target.value })} />
                </td>
                <td className="px-3 py-2">
                  <input className="table-input" type="number" value={point.x} onChange={(event) => updatePoint(point.id, { x: numberOrZero(event.target.value) })} />
                </td>
                <td className="px-3 py-2">
                  <input className="table-input" type="number" value={point.y} onChange={(event) => updatePoint(point.id, { y: numberOrZero(event.target.value) })} />
                </td>
                <td className="px-3 py-2">
                  <input
                    className="table-input"
                    min={1}
                    step={1}
                    type="number"
                    value={point.repetitions}
                    onChange={(event) => updatePoint(point.id, { repetitions: numberOrZero(event.target.value) })}
                  />
                </td>
                <td className="px-3 py-2">
                  <input
                    className="table-input"
                    min={0.01}
                    step={0.01}
                    type="number"
                    value={point.downSpeedMmPerSecond}
                    onChange={(event) => updatePoint(point.id, { downSpeedMmPerSecond: numberOrZero(event.target.value) })}
                  />
                </td>
                <td className="px-3 py-2">
                  <input
                    className="table-input"
                    min={0.01}
                    step={0.01}
                    type="number"
                    value={point.holdDownSeconds}
                    onChange={(event) => updatePoint(point.id, { holdDownSeconds: numberOrZero(event.target.value) })}
                  />
                </td>
                <td className="px-3 py-2">
                  <input
                    className="table-input"
                    min={0.01}
                    step={0.01}
                    type="number"
                    value={point.upSpeedMmPerSecond}
                    onChange={(event) => updatePoint(point.id, { upSpeedMmPerSecond: numberOrZero(event.target.value) })}
                  />
                </td>
                <td className="px-3 py-2">
                  <input
                    className="table-input"
                    min={0.01}
                    step={0.01}
                    type="number"
                    value={point.holdUpSeconds}
                    onChange={(event) => updatePoint(point.id, { holdUpSeconds: numberOrZero(event.target.value) })}
                  />
                </td>
                <td className="px-3 py-2 text-right">
                  <button className="icon-danger" type="button" onClick={() => removePoint(point.id)} title="Remover ponto" aria-label="Remover ponto">
                    <Trash2 size={17} />
                  </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </section>
  );
}
